...
def create_research_graph() -> StateGraph:
    ...
    def route_based_on_relevance(state: Dict[str, Any]) -> str:
        ...
        if new_iteration_count >= 3:
            print(f"Maksimum iterasiya sayına ({new_iteration_count}) çatıldı. Mövcud nəticələr əsasında hesabat yazılır.")
            return "write_research_report"
        
        # Otherwise, check if we should regenerate queries
        if state.get("should_regenerate_queries", False):
            print(f"İterasiya {new_iteration_count}: Axtarış sorğuları yenidən yaradılır.")
            return "generate_search_queries"
        else:
            print(f"İterasiya {new_iteration_count}: Axtarış nəticələri uyğundur. Hesabatın yazılmasına keçilir.")
            return "write_research_report"
...
if __name__ == "__main__":
    # Example usage
    question = "İçərişəhərin tarixi haqqında nə deyə bilərsən?"
