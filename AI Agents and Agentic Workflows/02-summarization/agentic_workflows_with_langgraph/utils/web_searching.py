...
def web_search(web_query: str, num_results: int) -> List[str]:
    ...
    if time_since_last_request < _min_request_interval:
        ...
        print(f"Sorğu limiti: Növbəti axtarış sorğusundan əvvəl {sleep_time:.2f} saniyə gözlənilir...")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if urls:
                ...
                print(f"Sorğu üçün nəticə tapılmadı: {web_query}")
            ...
            if attempt < max_retries - 1:
                ...
                print(f"{delay:.2f} saniyədən sonra yenidən sınanır... (Cəhd {attempt+1}/{max_retries})")
                time.sleep(delay)
            else:
                break
...
def fallback_search(query: str, num_results: int) -> List[str]:
    ...
    """
    print(f"Sorğu üçün ehtiyat axtarışdan istifadə edilir: {query}")
    ...
    stop_words = ["what", "where", "when", "why", "how", "is", "are", "was", "were", 
                 ...
                 "nə", "nədir", "harada", "nə vaxt", "niyə", "necə", "kim", "hansı",
                 "bu", "bir", "və", "ilə", "üçün", "haqqında", "olan", "olaraq",
                 "mən", "sən", "biz", "mənə", "de"]
    ...
    if len(query_words) >= 2:
        ...
        for i in range(len(query_words) - 1):
            ...
            wikipedia_urls.append(f"https://az.wikipedia.org/wiki/{topic}")
    
    # Add single word topics
    for word in query_words:
        if len(word) > 3:  # Only use meaningful words
            wikipedia_urls.append(f"https://az.wikipedia.org/wiki/{word.capitalize()}")
    
    # Try to create a direct Wikipedia search URL
    search_query = query_clean.replace(" ", "+")
    wikipedia_search_url = f"https://az.wikipedia.org/w/index.php?search={search_query}"
